...
#include <stdlib.h>
#include <string.h>
#include <hidapi/hidapi.h> // Changed from Linux headers
...
static void ouvrt_device_finalize(GObject *object)
{
    ...
    if (dev->fd != NULL) // Changed from -1
        hid_close(dev->fd); // Changed from close()
    free(dev->devnode);
    free(dev->name);
    free(dev->serial);
...
/*
 * Opens all file descriptors related to the device using HIDAPI.
 */
static int ouvrt_device_open_default(OuvrtDevice *dev)
{
    ...
    for (i = 0; i < 3; i++) {
        if (dev->fds[i] == NULL && dev->devnodes[i] != NULL) { // Changed from -1
            // Use HIDAPI to open the path instead of Linux open()
            dev->fds[i] = hid_open_path(dev->devnodes[i]);
            
            if (dev->fds[i] == NULL) { // Changed from -1
                g_print("%s: Failed to open HID path '%s'\n",
                    dev->name, dev->devnodes[i]);
                return -1;
            }
        }
    }
...
/*
 * Closes all file descriptors related to the device using HIDAPI.
 */
static void ouvrt_device_close_default(OuvrtDevice *dev)
{
    ...
    for (i = 2; i >= 0; i--) {
        if (dev->fds[i] != NULL) { // Changed from -1
            hid_close(dev->fds[i]); // Changed from close()
        }
        dev->fds[i] = NULL; // Changed from -1
    }
}
...
static void ouvrt_device_init(OuvrtDevice *self)
{
    ...
    // Initialize pointers to NULL instead of -1
    self->fds[0] = NULL;
    self->fds[1] = NULL;
    self->fds[2] = NULL;
