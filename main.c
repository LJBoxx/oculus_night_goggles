...
void clean(libusb_context *ctx, libusb_device_handle *dev, struct libusb_transfer *transfer, int interface_0, int interface_1) {
    if (transfer) {
...
int init()
{
    ...
    printf("%d\n", result);
    if (result != 0) {
        clean(ctx, dev, transfer, claim_0, claim_1);
        return 1;
    }
